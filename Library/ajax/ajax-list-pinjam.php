...
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_peminjaman WHERE siswa=\"$siswa\" AND kembali=\"0\"");
//buku belum kembali
...
if($_GET["kembali"]=="satu"){
    ...
    $kmb=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_peminjaman SET kembali='1', tgl_kembali='$kembaline' WHERE siswa='$siswa' && buku='$buku' && tgl_pinjam='$pinjam'");
    $buk=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_buku SET status='1', peminjam='0' WHERE peminjam='$siswa' && kd_buku='$buku'");
    if($kmb && $buk){
        $obuku=new buku();
        $osiswa=new siswa();
        $obuku->setKode($buku);
        $osiswa->setInduk($siswa);
        $s=mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM tbl_telat WHERE buku='$buku'");
        if($s){
            //echo "hapused";
        }else{
...
if($_GET["kembali"]=="all"){
    ...
    $kmb=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_peminjaman SET kembali='1', tgl_kembali='$kembaline' WHERE siswa='$siswa' && kembali='0'");
    $buk=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_buku SET status='1', peminjam='0' WHERE peminjam='$siswa'");
    ...
    //$tempo=
    if($kmb && $buk){
        $c=mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM tbl_telat WHERE induk ='$siswa'");
        if($c){
            //echo "Di Busek";
        }else{
