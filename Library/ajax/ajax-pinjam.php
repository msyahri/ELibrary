...
if($_GET["mode"]=="pinjam"){
    ...
    $ptgs=$_SESSION['uid'];
    //Sql Query
    $peminjaman=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tbl_peminjaman SET siswa=\"$dsiswa\", buku=\"$dbuku\", tgl_tempo=\"$tgl_kembali\", tgl_pinjam=\"$tgl_pinjam\", id_petugas=\"$ptgs\""); 
    //Debug
//    echo "INSERT INTO tbl_peminjaman SET siswa=\"$dsiswa\", buku=\"$dbuku\", tgl_kembali=\"2013-01-01\", tgl_pinjam=\"2013-01-01\", id_petugas=\"99\"";
    if($peminjaman){
        $buku=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_buku SET status=\"0\", peminjam=\"$dsiswa\", count=\"$count\" WHERE kd_buku=\"$dbuku\"");
        if($buku){
            $qSiswa=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET count=\"$meminjam\" WHERE no_induk=\"$dsiswa\"");
            if($qSiswa){
