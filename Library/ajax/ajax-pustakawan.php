...
?>
    ...
    <?php
    $ser=mysqli_query($GLOBALS["___mysqli_ston"], "select * from anggota");
    $n=1;
    while($us=mysqli_fetch_array($ser)){
        echo "<tr>";
        echo "<td>$n</td>";
        echo "<td>".$us['user']."</td>";
        echo "<td>".$us['nama']."</td>";
   ...
   $id=$_SESSION['uid'];
   $plama=$_POST['pwdlama'];
   $pbaru=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['wpdbaru']);
   $pulang=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['repeat']);
   $select=  mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * from tbl_pustakawan WHERE id=$id");
   ...
   if($_POST['ngirim']=='ya'){
       if($passwordlama==md5($plama)){
            if($pbaru==$pulang){
                $kunci=md5($pbaru);
                $suksess = mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_pustakawan SET kunci='$kunci' WHERE id='$id'");
                if($suksess){
