...
if(isset($_GET['hapus'])){
    $ide=$_GET['hapus'];
    mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM tbl_anggota WHERE no_induk='$ide'");
...
if($_GET['naik']=='kelas'){
    $lulusan=date('Y');
    $first=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"Alumni$lulusan\" WHERE kelas=\"X\"");
  
        $second=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"X\" WHERE kelas=\"VIII\"");
       
            $third=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"VIII\" where kelas=\"VII\"");
            
                $third1=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"VII\" where kelas=\"VI\"");
               
                    $third2=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"VI\" where kelas=\"V\"");
                   
                     $third3=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"V\" where kelas=\"IV\"");
                       
                            $third4=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"IV\" where kelas=\"III\"");
                         
                               
                                         $third6=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET kelas=\"II\" where kelas=\"I\"");
            ...
            if($third6){
                ...
                echo "FIRST ERROR: ".mysqli_error($GLOBALS["___mysqli_ston"]);
            }
...
$induk=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['no_induk']);
$nama=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['nama']);
$class=$_POST['kelas'];
$voc=$_POST['jurusan'];
if($_POST['tambah'] && $induk!='' && $nama!=''){
    $try=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tbl_anggota SET no_induk = '$induk', nama='$nama', kelas='$class', jurusan='$voc'");
    if($try){
            ...
            <form action='' method='get' name='cari'>
                ...
                <select name='jurusan' onchange='submiter()'>
                    ...
                    <optgroup label='Teknik'>
						...
						<option value='RPL'  <?php if($jur=='RPL') echo "selected='selected'"; ?>>Rekayasa Perangkat Lunak</option>
						<option value='TSM'  <?php if($jur=='TSM') echo "selected='selected'"; ?>>Teknik Sepeda Motor</option>
						<option value='TKR'  <?php if($jur=='TKR') echo "selected='selected'"; ?>>Teknik Kendaraan Ringan</option>
						<option value='TAV'  <?php if($jur=='TAV') echo "selected='selected'"; ?>>Teknik Audio Video</option>
