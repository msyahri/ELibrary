...
if($_SESSION['level']=='Admin'){
    ...
    //echo $sejenis2;
    sambung();
    $sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_buku WHERE kd_buku LIKE '$sejenis2%' ORDER BY kd_buku DESC");
    //$all=mysql_query("select * from buku where kd_buku like '$sejenis2%' && kd_buku not like '$not%'");
    $yang_sama=mysqli_num_rows($sql);
    //$all=mysql_num_rows($all);
    if($yang_sama != '0' && $_GET['tBuku']==''/*&& !isset($_SESSION['tBuku'])*/){
        
        $has=mysqli_fetch_array($sql);
    ...
    if($_POST['tambah'] && $_POST['kd_buku']!=''){
        $kode=$_POST['kd_buku'];
        $jdl=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['judul']);
        $pngrng=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['pengarang']);
        $thn=$_POST['tahun'];
        $pnrbt=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_POST['penerbit']);
        $hrg=$_POST['harga'];
        sambung();
        $sql=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tbl_buku SET kd_buku='$kode', judul='$jdl', pengarang='$pngrng', thn_terbit='$thn', penerbit='$pnrbt', harga='$hrg', status='1'");
    ...
    if($_GET['hapus']=='1'){
        $bukune=$_GET['buku'];
        $guak=mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM tbl_buku WHERE kd_buku='$bukune'");
        if($guak){
        ...
        $bukunya=$_GET['tBuku'];
            $by=explode('/',$bukunya);
                $kod=substr($by[2],0,2);
                    ...
                    //echo $bukunya;
        $sq=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_buku WHERE kd_buku LIKE '$bukunya%' ORDER BY kd_buku DESC");
