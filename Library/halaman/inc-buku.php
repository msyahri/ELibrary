...
$key=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['pencarian']);
$where=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['where']);
$order=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['order']);
$stat=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['status']);
$bates=$_GET['banyak'];
putus();
if($where==''){
  $where='kd_buku';  
...
$a=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_buku WHERE $where LIKE '%$key%' && status LIKE '$stat' ORDER BY $order ASC");
//Debug Program
//echo "SELECT * FROM tbl_buku WHERE $where LIKE '%$key%' && status LIKE '$stat' ORDER BY $order ASC";
$semua=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_buku");
...
<?php
sambung();
$a=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_buku WHERE $where LIKE '%$key%' && status LIKE '$stat' ORDER BY $order ASC LIMIT $mulai, $bts");
//Debug Program
//echo "SELECT * FROM tbl_buku WHERE $where LIKE '%$key%' && status LIKE '$stat' ORDER BY $order ASC LIMIT $mulai, $bts";
$no=1;
$chek=mysqli_num_rows($a);
if($chek==1){
  ?>
  <script type="text/javascript">
