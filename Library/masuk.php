...
<html>
<head>
<title>Login | Perpustakaan Manusa</title>
...
if ($pencet != '' && $nama != '' && $kunci != ''){
    $s=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_pustakawan WHERE user='".$nama."' AND kunci='".$kunci."'");
    $c=mysqli_num_rows($s);
    if($c == '1'){
      $t= mysqli_fetch_array($s);
      $log=$t['login'];
      $log=$log+1;
      mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_pustakawan SET login='$log' WHERE user='$nama'");
