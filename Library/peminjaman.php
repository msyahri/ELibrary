...
if($_GET['tanggal']!=''){
    $t=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['tanggal']);
    $tgu=$t;
}else{
    $t='';
}
if($_GET['bulan']!=''){
    $b=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['bulan']);
}else{
    $b='';
}
if($_GET['tahun']!=''){
    $th=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_GET['tahun']);
}else{
    $th='';
}
$jajal="$th-$b-$t";
...
$dari=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_peminjaman");
$dari=mysqli_num_rows($dari);
$belum=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_peminjaman WHERE kembali='0'");
$belum=mysqli_num_rows($belum);


/*Lunasi dendo*/
if($_GET['denda']=='lunas'){
    $hapus=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET denda='0' WHERE no_induk='$cari'");
    ...
    if($val!='0'){
        ...
        mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tbl_kas SET masuk = '$val', tgl = '$skg', ket='Denda dari $convert', saldo='$saldo'"); 
    }
    if($hapus){
...
if($_POST['denda'] && $_POST['tempo'] != ''){
    ...
    if($cval!='-'){
        mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_anggota SET denda ='$value' WHERE no_induk='$cari'");
...
if(isset($_POST['semua'])){
    $kmb=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_peminjaman SET kembali='1', tgl_kembali='$tgl_kembali' WHERE siswa='$cari' && kembali='0'");
    $buk=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_buku SET status='1', peminjam='0' WHERE peminjam='$cari'");
    //$tempo=
    if($kmb && $buk){
        $c=mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM tbl_telat WHERE induk ='$cari'");
        if($c){
            //echo "Di Busek";
        }else{
...
if(isset($_GET['kmb'])){
    ...
    $kmb=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_peminjaman SET kembali='1', tgl_kembali='$tgl_kembali' WHERE siswa='$cari' && buku='$kd_buku' && tgl_pinjam='$tgl_kmbl'");
    $buk=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_buku SET status='1', peminjam='0' WHERE peminjam='$cari' && kd_buku='$kd_buku'");
    //$tempo=
    if($kmb && $buk){
        $s=mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM tbl_telat WHERE buku='$kd_buku'");
        if($s){
            //echo "hapused";
        }else{
            echo "Gagal Menghapus Jatuh Tempo !!";
...
/*Ambil Peminjam*/
if($cari != '' && $where=='siswa'){
    $pem=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_anggota WHERE no_induk = $cari");
    $si=mysqli_fetch_array($pem);
    $hisi=mysqli_num_rows($pem);
}
...
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_peminjaman WHERE $where LIKE '%$cari%' && kembali LIKE '$kmbl'");
$semua=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_peminjaman");
$semua=mysqli_num_rows($semua);
...
$sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT *, DATE_FORMAT( tgl_pinjam,  '%d %M %Y' ) AS format_pinjam, DATE_FORMAT( tgl_tempo,  '%d %M %Y' ) AS format_tempo, DATE_FORMAT( tgl_kembali,  '%d %M %Y' ) AS format_kembali FROM tbl_peminjaman WHERE $where LIKE '%$cari%' && kembali LIKE '$kmbl' && tgl_pinjam LIKE '%$jajal%' ORDER BY siswa ASC limit $mulai, $bts");
...
$ht=mysqli_num_rows($sql);
//
$sqll=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_peminjaman WHERE $where LIKE '%$cari%' && kembali LIKE '$kmbl' && tgl_pinjam LIKE '%$jajal%' ORDER BY siswa");
$ht2=mysqli_num_rows($sqll);
...
<table width='100%' cellpadding='2' cellspacing='1' border='0' id='peminjaman' style='border: 1px solid #DDD7BD; font-size: 12px;' class="table table-hover table-striped">
    ...
    <?php $book=new buku(); ?>
    <?php $i=1; $kembali=0; while($p=mysqli_fetch_array($sql)){
        $book->setKode($p['buku']);
