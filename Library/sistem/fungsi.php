...
function catat($us,$text){
    ...
    $peng=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $us);
    $aksi=mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $text);
        $insert=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO log SET user = '$peng', aksi = '$aksi', tgl = '$now'");
        if($insert){
...
class db{
    ...
    public function sambungDB(){
        @($GLOBALS["___mysqli_ston"] = mysqli_connect($this->h, $this->u, $this->p));
        @mysqli_select_db($GLOBALS["___mysqli_ston"], $this->d);
    }
    public function sql($query){
        //$this::sambungDB();
        
        $this->query=mysqli_query($GLOBALS["___mysqli_ston"], $query);
    }
    ...
    public function baris($sql){
        //$this::sambungDB();
        $this->baris=mysqli_query($GLOBALS["___mysqli_ston"], $sql);
...
class exel{
    ...
    public function doExport($sql){
        ...
        $rec = mysqli_query($GLOBALS["___mysqli_ston"], $sql) or die (mysqli_error($GLOBALS["___mysqli_ston"]));
...
function get_sistem($opt){
    $sql=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_config WHERE param='$opt'");
...
function set_sistem($inpt, $val){
    $s=mysqli_query($GLOBALS["___mysqli_ston"], "UPDATE tbl_config SET value='$val' WHERE param='$inpt'");
    if(!$s){
...
function get_nama($id){
    $s=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_anggota WHERE no_induk=\"$id\"");
    $r=mysqli_fetch_array($s);
    
    return $r["nama"];
}
function get_pustakawan($id){
    $s=  mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tbl_pustakawan WHERE id=\"$id\"");
    $r= mysqli_fetch_array($s);
    
    return $r['nama'];
}
function id_pustakawan($nama){
    $s=  mysqli_query($GLOBALS["___mysqli_ston"], "SELECT id FROM anggota WHERE nama=\"$nama\"");
    $r= mysqli_fetch_array($s);
