...
exit();
   ...
   //Buka tabel buku lama
    $sql_buku=  mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM buku");
    //Kosongkan database baru
    mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE tbl_buku");
    //Buat Counter
    $berhasil=0;
    $gagal=0;
    while($data=  mysqli_fetch_array($sql_buku)){
        ...
        $transfer=mysqli_query($GLOBALS["___mysqli_ston"], $transfer_sql);
        if($transfer){
    ...
    $sql_peminjaman= mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM pinjaman");
    //Kosongkan database baru
    mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE tbl_peminjaman");
    //Buat Counter
    $berhasil=0;
    $gagal=0;
    while($data=  mysqli_fetch_array($sql_peminjaman)){
        //Data BUku Lama
       ...
       //echo "$sql_transfer <br>"; 
        $transfer=  mysqli_query($GLOBALS["___mysqli_ston"], $sql_transfer);
        if($transfer){
            //echo "$no Berhasil di transfer<br>";
            $berhasil++;
        }else{
    ...
    //Just Rename thok.. :D
    mysqli_query($GLOBALS["___mysqli_ston"], "DROP TABLE IF EXISTS tbl_anggota");
    $sql_first=  mysqli_query($GLOBALS["___mysqli_ston"], "RENAME TABLE siswa TO tbl_anggota");
    $sql_second = mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE tbl_anggota CHANGE count_pinjam count INT(11) NOT NULL DEFAULT '0'");
    if($sql_first){
        if(!$sql_second){
            //echo "ERROR QUERY 2 : change count_pinjam to count";
            $berhasil.="ERROR QUERY 2 : change count_pinjam to count ".mysqli_error($GLOBALS["___mysqli_ston"]);
            if(mysqli_query($GLOBALS["___mysqli_ston"], "ALTER TABLE `tbl_anggota` ADD `alumni` TINYINT NOT NULL AFTER `jurusan`")){
                //Done
            }else{
        ...
        $berhasil= "ERROR QUERY 1: rename table ".mysqli_error($GLOBALS["___mysqli_ston"]);
    }
    ...
    $status=0;
    //**Tabel Config
    if(mysqli_query($GLOBALS["___mysqli_ston"], "DROP TABLE IF EXISTS tbl_config")){
        if(mysqli_query($GLOBALS["___mysqli_ston"], "RENAME TABLE sistem TO tbl_config")){
    ...
    //**Tabel Anggota
    if(mysqli_query($GLOBALS["___mysqli_ston"], "DROP TABLE IF EXISTS tbl_pustakawan")){
        if(mysqli_query($GLOBALS["___mysqli_ston"], "RENAME TABLE anggota TO tbl_pustakawan")){
    ...
    //**Tabel Kas
    $query=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM kas");
    ...
    while($data=  mysqli_fetch_array($query)){
        ...
        if(mysqli_query($GLOBALS["___mysqli_ston"], $convert_sql)){
            $berhasil++;
        }else{
    ...
    if($berhasil!=0){
        $finish_query=  mysqli_query($GLOBALS["___mysqli_ston"], "DROP TABLE buku, kas, pinjaman,tempo");//Daffuk we
        if($finish_query){
            catat("SISTEM","Berhasil Upgrade ke versi ".getVersion());
            echo "<a href='?upgrade=finish' class='btn btn-success'>Selesai >></a>";
        }else{
            echo "Unable to drop old tables, do it manualy :D<br>". mysqli_error($GLOBALS["___mysqli_ston"]);
        }
